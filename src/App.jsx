...
const Loading = ({
  texto = "Carregando...",
  corGiro = "text-indigo-600",
  corSombra = "bg-indigo-500",
  tamanho = "w-10 h-10",
  overlay = false,
}) => {
  const conteudo = (
    <div className="flex flex-col items-center justify-center gap-4">
      <div className="relative flex items-center justify-center">
        <div
          className={`absolute inset-0 rounded-full blur-xl opacity-40 animate-pulse ${corSombra}`}
        ></div>
        <Loader2
          className={`relative z-10 animate-spin ${tamanho} ${corGiro}`}
        />
      </div>
      {texto && (
...
const InputClean = ({ label, name, placeholder, value, onChange }) => (
  <div className="flex flex-col">
    <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1 ml-1">
      {label}
    </label>
    <input
      name={name}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const eventosValidos = listaEventos.filter(
      (ev) => ev.expiracao === "permanente" || ev.expiracao >= hoje,
    );
  ...
  const buscarPorDoSol = async (dataSabado) => {
    ...
    try {
      const res = await fetch(
        `https://api.sunrise-sunset.org/json?lat=${lat}&lng=${lng}&date=${dataISO}&formatted=0`,
      );
      const apiData = await res.json();
      const sunsetUtc = new Date(apiData.results.sunset);
      const horarioSP = sunsetUtc.toLocaleTimeString("pt-BR", {
  ...
  const gerarBoletim = async () => {
    ...
    eventos.forEach(
      (ev) =>
        (textoEventos += `${ev.texto.replace("{DATA_DOMINGO}", formatarData(datas.domingo))}\n\n`),
    );

    // 1. Filtra os aniversariantes do mês atual
    const niversMes = aniversariantesJSON.filter(
      (p) => p.mes === dataBase.getMonth() + 1,
    );

    // 2. Ordena os aniversariantes pela data (dia)
    niversMes.sort((a, b) => {
      // Extrai o dia (os 2 primeiros caracteres antes da barra) e converte para número
      const diaA = parseInt(a.dia.split("/")[0]);
      const diaB = parseInt(b.dia.split("/")[0]);
      return diaA - diaB;
    });

    // 3. Monta a string do boletim
    let niversStr = `🟤 *Lista de Aniversariantes do mês*\n`;
    if (niversMes.length > 0) {
      niversMes.forEach((p) => (niversStr += `${p.dia} - _${p.nome}_\n`));
    } else {
  ...
  const copiarTexto = () => {
    navigator.clipboard.writeText(boletim);
    setCopiado(true);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-100 via-indigo-50 to-teal-50 pb-24 font-sans selection:bg-indigo-200">
      <header className="pt-10 pb-6 px-6 text-center">
        ...
        <p className="text-slate-500 text-sm mt-1">
          Crie informativos semanais rapidamente.
        </p>
      </header>
      ...
      <main className="px-4 max-w-md mx-auto">
        <div
          className={
            abaAtiva === "editor" ? "block animate-in fade-in" : "hidden"
          }
        >
          <Card
            titulo="Modelo do Boletim"
            icone={<LayoutTemplate className="w-5 h-5 text-indigo-500" />}
          >
            <select
              value={template}
          ...
          <Card
            titulo="Sábado"
            icone={<CalendarDays className="w-5 h-5 text-indigo-500" />}
          >
            <div className="flex gap-2">
              <div className="flex-1">
                <InputClean
                  label="Pregador"
                  name="sabadoPregador"
                  value={formData.sabadoPregador}
                  onChange={handleInputChange}
                  placeholder="Ex: Victor Matheus"
                />
              </div>
              <button
                onClick={() => setModalAtivo("sabado")}
                className="mt-5 bg-white shadow-sm border border-slate-200 text-slate-600 rounded-xl w-12 flex items-center justify-center hover:bg-slate-50 active:scale-95 transition-all"
              >
          ...
          <Card
            titulo="Domingo"
            icone={<BookOpen className="w-5 h-5 text-indigo-500" />}
          >
            <div className="flex gap-2">
              <div className="flex-1">
                <InputClean
                  label="Pregador"
                  name="domingoPregador"
                  value={formData.domingoPregador}
                  onChange={handleInputChange}
                  placeholder="Ex: Luís Gonçalves"
                />
              </div>
              <button
                onClick={() => setModalAtivo("domingo")}
                className="mt-5 bg-white shadow-sm border border-slate-200 text-slate-600 rounded-xl w-12 flex items-center justify-center hover:bg-slate-50 active:scale-95 transition-all"
              >
                <Settings2 className="w-5 h-5" />
              </button>
          ...
          <Card
            titulo="Quarta-feira"
            icone={<Heart className="w-5 h-5 text-indigo-500" />}
          >
            <div className="flex gap-2">
              <div className="flex-1">
                <InputClean
                  label="Pregador"
                  name="quartaPregador"
                  value={formData.quartaPregador}
                  onChange={handleInputChange}
                  placeholder="Ex: Pr. Peduti"
                />
              </div>
              <button
                onClick={() => setModalAtivo("quarta")}
                className="mt-5 bg-white shadow-sm border border-slate-200 text-slate-600 rounded-xl w-12 flex items-center justify-center hover:bg-slate-50 active:scale-95 transition-all"
              >
                <Settings2 className="w-5 h-5" />
              </button>
          ...
          <Card
            titulo="Próximo Sábado"
            icone={<SkipForward className="w-5 h-5 text-indigo-500" />}
          >
            <InputClean
              label="Pregador"
              name="proximoSabadoPregador"
              value={formData.proximoSabadoPregador}
              onChange={handleInputChange}
              placeholder="Ex: Wellington IASD"
            />
          </Card>

          <Card
            titulo="Eventos & Anúncios"
            icone={<Bell className="w-5 h-5 text-indigo-500" />}
          >
            <div className="bg-white/50 p-3 rounded-2xl border border-white/60 mb-2 transition-all focus-within:bg-white focus-within:shadow-sm">
              <textarea
                ...
                className="w-full bg-transparent border-0 focus:ring-0 text-sm resize-none text-slate-700 placeholder:text-slate-400 outline-none"
                rows={2}
              />
              <div className="flex justify-between items-center mt-2 border-t border-slate-200/50 pt-2">
                ...
                <button
                  ...
                  className="bg-indigo-50 text-indigo-600 flex items-center gap-1 hover:bg-indigo-100 text-xs font-bold px-3 py-1.5 rounded-lg transition-colors"
                >
            ...
            <div className="space-y-2 mt-2">
              {eventos.map((ev) => (
                <div
                  key={ev.id}
                  className="flex justify-between items-center bg-white/80 p-3 rounded-xl text-xs border border-slate-100 shadow-sm"
                >
                  <span className="truncate max-w-[80%] text-slate-600 font-medium">
                    {ev.texto.split("\n")[0]}
                  </span>
                  <button
        ...
        <div
          className={
            abaAtiva === "resultado" ? "block animate-in fade-in" : "hidden"
          }
        >
          <div className="bg-white/80 backdrop-blur-md rounded-3xl p-5 shadow-lg border border-white mb-6 relative">
            <div className="flex justify-between items-center mb-4">
              ...
              {boletim && (
                <button
                  onClick={copiarTexto}
                  className={`px-4 py-2 flex items-center gap-2 rounded-full text-xs font-bold text-white transition-all shadow-md ${copiado ? "bg-emerald-500 shadow-emerald-200" : "bg-indigo-500 shadow-indigo-200 active:scale-95"}`}
                >
                  {copiado ? (
                    <>
          ...
          <div className="bg-white w-full sm:max-w-sm rounded-t-3xl sm:rounded-3xl p-6 pb-10 shadow-2xl animate-in slide-in-from-bottom-10 border border-slate-100">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-bold text-xl text-slate-800 capitalize flex items-center gap-2">
                <Settings2 className="w-5 h-5 text-indigo-500" /> Detalhes:{" "}
                {modalAtivo}
              </h3>
              <button
            ...
            <div className="flex flex-col gap-4">
              ...
              {modalAtivo === "domingo" && (
                <InputClean
                  label="Localização"
                  name="domingoLocal"
                  value={formData.domingoLocal}
                  onChange={handleInputChange}
                  placeholder="Ex: IASD Central Diadema"
                />
              )}
              {modalAtivo === "quarta" && (
                <InputClean
                  label="Livro/Estudo"
                  name="quartaEstudo"
                  value={formData.quartaEstudo}
                  onChange={handleInputChange}
                  placeholder="Ex: Livro Oração"
                />
              )}
              <button
                disabled={carregando}
                onClick={() => setModalAtivo(null)}
                className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3.5 rounded-xl mt-4 shadow-md shadow-indigo-200 transition-all active:scale-95"
              >
                Salvar e Fechar
              </button>
            </div>
